...
class UE(Node):
    def __init__(self, env, comp_cap, p_calc, p_send, max_task) -> None:
        super().__init__(env, comp_cap)
        self.mec_conns = []
        self.tasks = Queue(max_task)
        # CPU power consumption in watts
        self.p_calc = p_calc
        # transmission power in Hz
        self.p_send = p_send
        self.agent = Agent(state_dim=((len(self.env.mec_servers) * 2) + (self.env.task_count * 2) + 2), action_dim=((2 + len(self.env.mec_servers)) * self.env.task_count), actor_lr=self.env.actor_lr, critic_lr=self.env.critic_lr, discount=self.env.discount)
    ...
    def calculate_high_state(self):
        self.high_state = np.append(
            [self.env.snr['max']] * len(self.mec_conns), [self.env.task_count] * len(self.mec_conns))
        self.high_state = np.append(self.high_state, self.env.task_count)
        temp = [self.env.max_task_data_size_req] * self.env.task_count
        self.high_state = np.append(self.high_state, temp)
        temp = [self.env.max_task_cycles_req] * self.env.task_count
        self.high_state = np.append(self.high_state, temp)
        self.high_state = np.append(self.high_state, self.env.task_count)
    
    def calculate_low_state(self):
        self.low_state = np.append([self.env.snr['min']] * len(self.mec_conns), [0] * len(self.mec_conns))
        self.low_state = np.append(self.low_state, 0)
        temp = [self.env.min_task_data_size_req] * self.env.task_count
        self.low_state = np.append(self.low_state, temp)
        temp = [self.env.min_task_cycles_req] * self.env.task_count
        self.low_state = np.append(self.low_state, temp)
        self.low_state = np.append(self.low_state, 0)

    def calculate_state(self):
        self.state = np.append([conn.snr for conn in self.mec_conns], [len(conn.mec_server.tasks) for conn in self.mec_conns])
        self.state = np.append(self.state, self.tasks.get_len())
        temp = [task.size for task in self.tasks.list]
        temp = np.append(temp, np.zeros(self.env.task_count - len(temp)))
    ...
    def get_normalized_state(self):
        ...
        print("shaep: {}".format(len(self.state)))
        print("high state: {}".format(self.high_state))
        print("shaep: {}".format(len(self.high_state)))
        print("low state: {}".format(self.low_state))
        print("shaep: {}".format(len(self.low_state)))
    ...
    def take_step(self, action):
        action = (action + 1) / 2
        single_task_dim = len(action) / self.env.task_count
        reward = 0.0
        for a in self.yield_task_action(action, int(single_task_dim)):
            ...
            if task != None:
                if decision == 0:
                    ...
                    curr_reward = self.env.time_impo * comp_delay + self.env.energy_impo * energy_consum
                    reward += curr_reward
                    self.poll_task()
                    ...
                    curr_reward = self.env.time_impo * (comp_delay + trans_delay) + self.env.energy_impo * energy_consum
                    reward += curr_reward
                    self.poll_task()
                    print("reward: {}".format(curr_reward))
                else:
                    ...
                    if len(self.mec_conns[MEC_index].mec_server.tasks) < self.env.task_count:
    ...
    def process(self):
        ...
        s_, r = self.take_step(action)
        self.agent.feedback(s, r, s_, [action])
        self.agent.save()
        return r

class MEC(Node):
    def __init__(self, env, comp_cap, max_task) -> None:
        super().__init__(env, comp_cap)
        self.tasks = {}
    ...
    def add_task(self, task):
        ...
        self.tasks[id] = task
        self.scheduler.add_job(self.complete_task, args=(id,), trigger=DateTrigger(after_secs), max_instances=1)
